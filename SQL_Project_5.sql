...
--Question12. Is there a correlation between higher salaries and the presence of benefits in job postings?
SELECT 
    CASE 
        WHEN b.job_id IS NOT NULL THEN 'With Benefits'
        ELSE 'Without Benefits'
    END AS benefits_status,
    COUNT(s.job_id) AS job_count,
    ROUND(AVG(s.min_salary), 2) AS avg_min_salary,
    ROUND(AVG(s.med_salary), 2) AS avg_med_salary,
    ROUND(AVG(s.max_salary), 2) AS avg_max_salary
FROM job.salary s
LEFT JOIN job.benefits b ON s.job_id = b.job_id
GROUP BY benefits_status
